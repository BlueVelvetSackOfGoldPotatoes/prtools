...
%   ALF Parameter determining the dimension and the mapping (optional, defaulf: Inf)
%       (0,1) - fraction of the total magnitude of the preserved variance
%       Inf   - no dimension reduction; keeping all dimensions (it's noisy)
...
%
% REFERENCE
% 1. B. Schölkopf, A. Smola, and K.-R. Müller. Kernel principal component analysis.
...
% This happens when SELEIGS is called with the paremeter 'CUT'.

if nargin < 3 | isempty(prec), prec = 1e-4; end
...
B = -repmat(1/n,n,n);
B(1:n+1:end) = B(1:n+1:end) + 1;  % B = eye(n) - ones(n,n)/n
K  = B * K * B;            % K is now a centered kernel
...
% Eigenvalues are noe sorted by decreasing absolute value

[J,sig] = seleigs(l,alf,prec);  % J is the index of selected eigenvalues
L = l(J);                       % Eigenvalues
Q = Q(:,J);                     % Eigenvectors

% Normalize Q such that the eignevectors of the corresponding
