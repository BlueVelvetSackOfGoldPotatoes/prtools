    ...
    if strcmp(a.type,'raw')    % this is a raw datafile
      ...
      prprogress([],'%5i\n',0);
       	...
       	prprogress([],'%5i\n',j);
				...
				L = find([fnames.isdir] == 0); % throw out subdirs
		...
		elseif strcmp(a.type,'half-baked') | strcmp(a.type,'pre-cooked') 
			...
			for j=1:n
				...
				if isempty(ident)
					...
					for j=1:length(identfields)
						f  = ident.(identfields{j});
						fb = bident.(identfields{j});
						ident.(identfields{j}) = [f; fb];
					end
				end
			end
			a.files = files;
...
fp: /home/goncalo/Documents/RUG/TA_Positions/BioCS/BioSB/BioSB-MachineLearning-2020/website/Data_and_code/Data and code/prtools4112/@datafile/setfiles.m
 fn: setfiles -------------
