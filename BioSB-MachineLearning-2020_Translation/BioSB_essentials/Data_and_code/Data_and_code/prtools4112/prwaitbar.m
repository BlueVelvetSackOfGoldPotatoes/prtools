...
if decrease_depth                         % update admin
	...
	if DEPTH == 0                           % take care that new waitbar starts fresh
		...
		if ~isempty(p), set(p(2),'erasemode','normal'); end %Amin: a patch to work with new matlabs
		waitbar(0,WHANDLE,sprintf(' \n \n \n' ));
		set(WHANDLE,'name',['                         PRWAITBAR:      ']);
        if ~isempty(p), set(p(2),'erasemode','none'); end %Amin: a patch to work with new matlabs
		prwaitbar;  
...
fp: /home/goncalo/Documents/RUG/TA_Positions/BioCS/BioSB/BioSB-MachineLearning-2020/website/Data_and_code/Data and code/prtools4112/prwaitbar.m
 fn: prwaitbar -------------
