        ...
        if (length(featset)==1) & (isstr(b)) 	
  				...
  				for sub = featset 							% Copy feature J: A(:,sub) = B(:,J).
  					...
  					if (isa(b,'dataset'))
  						...
  						a.data(:,sub)    = b(:,j);			
  						testfeatdom(a,j);						% Test whether features fit domain.
  					end
  				end
  			end
  		end
    ...
    if (strcmp(s.subs{2},':')) 						% A(row,:) = .....
  	...
  	if (~isempty(col)) & (~isempty(row))
  		if (length(col) == 1) & (isstr(b))  % Single feature with nominal value.
  			[data,nb,domain] = renumlab(b,a.featdom{col});
				
				% LENGTH(NB) is the number of feature names we already had. New data 
				% should fit in that.
  			if (max(data) > length(nb))
  				error('Unknown feature name detected')
  			end
...
fp: /home/goncalo/Documents/RUG/TA_Positions/BioCS/BioSB/BioSB-MachineLearning-2020/website/Data_and_code/Data and code/prtools4112/@dataset/subsasgn.m
 fn: subsasgn -------------
